...
class artigo_listar
{
    ...
    public static function listar()
    {
        ...
        $pages = 0;
        if ($receivedData != "") {
            if (isset($dataJSON->titulo)) {
                $inputs['titulo_pesquisar'] = preg_replace('/[^\p{L}\p{N}\s\-]/u', '', $dataJSON->titulo);
            }
            ...
            if (isset($dataJSON->entrada)) {
                ...
                $pages = artigodao::getCountPage($dataJSON->entrada, $inputs);
                $total =  artigodao::getCountPagination($inputs);
                $operator = artigodao::getFindByAllPagination($inputs);
                $recid = $start_limit;
                $sis_all_data = [];
                ...
                foreach ($operator as $op) {
                    $array = [
                        ...
                        'titulo' => $op->getTitulo(),
                        'resumo' => $op->getResumo(),
                        'conteudo' => $op->getConteudo(),
                        'status' => $op->getStatus(),
                        'enviado_em' => $op->getEnviado_em(),
                        'atualizado_em' => $op->getAtualizado_em(),
                    ];
