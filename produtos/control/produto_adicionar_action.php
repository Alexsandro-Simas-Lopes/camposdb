<?php require_once("../../produtos/model/produtodao.php");
...
class produto_adicionar {
    public function __construct()
    {
        ...
        if (!empty($received['name'])) {
            $verifica_existe = produtodao::verifica_existe_produto($received['name'], $received['price'], $received['marca'], $received['categoria'], $received['sub_categoria']);
            if (!empty($verifica_existe->getId())) {
                echo $error_code;  // Produto já existe
            } else {
                ...
                // Usa a URL da imagem que já foi processada pelo upload.php
